...
const goalLabels = { filedownload: 'Download Arquivo', filedelete: 'Deletar Arquivo', filechange: 'Alterar Arquivo', getadmin: 'Obter Admin', getstring: 'Obter String', delay: 'Esperar', hasflag: 'Verificar Flag' };
...
export default function Missions() {
  ...
  return (
    <div className="min-h-screen">
      <TerminalHeader
        title="Missões"
        subtitle="Objetivos e contratos da sua extensão"
        actions={
          <div className="flex items-center gap-3">
            ...
            {selectedExt && missions.length > 0 && (
              <div className="flex items-center bg-secondary rounded-md p-0.5 border border-border">
                ...
                >
                  <GitBranch className="w-3.5 h-3.5" /> Grafo
                </button>
              </div>
            )}
      ...
      {viewMode === 'graph' && selectedExt ? (
        ...
        <div className="p-6">
          {!selectedExt ? (
            <EmptyState icon={Target} title="Selecione uma extensão" description="Escolha uma extensão para gerenciar missões." />
          ) : missions.length === 0 && !isLoading ? (
            <EmptyState icon={Target} title="Nenhuma missão" description="Crie missões para sua extensão." actionLabel="Nova Missão" onAction={() => setShowForm(true)} />
          ) : (
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 ml-6 text-xs font-mono text-muted-foreground">
                          <Badge className="bg-accent/20 text-accent text-[10px]">{goalLabels[m.goal_type] || m.goal_type}</Badge>
      ...
      <Dialog open={showForm || !!editingMission} onOpenChange={closeDialog}>
        <DialogContent className="bg-card border-border sm:max-w-xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="font-sans">{editingMission ? 'Editar Missão' : 'Nova Missão'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label className="font-mono text-xs">ID da Missão</Label>
              ...
              <div>
                <Label className="font-mono text-xs">Título</Label>
            ...
            <div>
              <Label className="font-mono text-xs">Título do Posting</Label>
            ...
            <div>
              <Label className="font-mono text-xs">Texto do Board (Posting)</Label>
            ...
            <div className="border border-border rounded-lg p-3 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <Label className="font-mono text-xs">Remetente</Label>
                ...
                <div>
                  <Label className="font-mono text-xs">Assunto</Label>
              ...
              <div>
                <Label className="font-mono text-xs">Corpo</Label>
            ...
            <div className="border border-border rounded-lg p-3 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Select value={form.goal_type} onValueChange={(v) => updateForm('goal_type', v)}>
                    ...
                    <SelectContent>{goalTypes.map(g => <SelectItem key={g} value={g} className="font-mono text-xs">{goalLabels[g]}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                <div>
                  <Label className="font-mono text-xs">Nó Alvo</Label>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <Label className="font-mono text-xs">Arquivo</Label>
                ...
                <div>
                  <Label className="font-mono text-xs">Caminho</Label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label className="font-mono text-xs">Ação de Início da Missão</Label>
              ...
              <div>
                <Label className="font-mono text-xs">Ação de Fim da Missão</Label>
            ...
            <div className="border border-border rounded-lg p-3 space-y-3">
              <Label className="font-mono text-xs">Anexos de Email (linhas separadas)</Label>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                <Label className="font-mono text-xs">Próxima Missão</Label>
              ...
              <div>
                <Label className="font-mono text-xs">Rank Req.</Label>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSubmit} disabled={!form.mission_id || !form.title} className="font-mono text-xs">{editingMission ? 'Salvar' : 'Criar'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      <DeleteConfirmDialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={() => { deleteMut.mutate(deleteTarget.id); setDeleteTarget(null); }} title={`Excluir missão "${deleteTarget?.title}"?`} />
    </div>
  );
}